# `scripts/bench/pt` — inference latency

Per-forward-pass latency at batch 1 with an explicit device sync each iteration.

    python scripts/bench/pt/run.py --model canvit --device cuda --scene-px 512 --dtype amp-bf16
    python scripts/bench/pt/analyze.py --pattern 'scripts/bench/pt/results/*.jsonl'
    python scripts/bench/pt/matrix.py --help     # sweep driver: one subprocess per cell
