...
cd $R/canvit && PYTHONSAFEPATH=1 PYTHONPATH="$S/pytorch:$S/pretrain" \
  .venv-cu126/bin/python claude_dev/unification/april_equivalence/parity_probe_april.py out.json
```
...
(`3d7eb1f`) had broken it, along with 21 other scripts under `claude_dev/unification/`, because
