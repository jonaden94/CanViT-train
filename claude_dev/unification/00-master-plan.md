...
canvit_train (the merged training repo)
  ...
  claude_dev/unification/        this plan + per-phase implementation notes
...
| **P6** | Archival: mine RL repo docs (26 session docs, results, preserved-checkpoints) into `claude_dev/unification/migrated/`; archive specialize + RL on GitHub; update session CLAUDE.md + git_status_all.sh | nothing references the archived repos; published HF policy checkpoints still load via core |
...
`docs/unification-status.md` are copied into `claude_dev/unification/migrated/` **before**
