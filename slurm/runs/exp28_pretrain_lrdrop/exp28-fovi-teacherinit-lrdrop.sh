#!/bin/bash
# exp28 PHASE B — the x0.1 LR drop for exp28-fovi-teacherinit.
#
# DO NOT SUBMIT until exp28-fovi-teacherinit has actually produced step-1130496.pt. Phase A's
...
#     optimizer/scheduler/step=0; NEW wandb run (parent untouched). Parent step k <-> k-1130496.
...
# === ESSENTIALS ===
RUN_GROUP=exp28_pretrain_lrdrop
RUN_NAME=exp28-fovi-teacherinit-lrdrop
...
SEED_CKPT=/mnt/vast-nhr/projects/nib00021/jonathan/repos/canvit/logs/exp28_pretrain_lrdrop/exp28-fovi-teacherinit/checkpoints/step-1130496.pt
if [ ! -f "$SEED_CKPT" ]; then
    echo "REFUSING: $SEED_CKPT does not exist yet."
    echo "Phase A (exp28-fovi-teacherinit) has not reached step 1130496."
...
CFG_WANDB_PROJECT=exp28_pretrain_lrdrop
