#!/bin/bash
# exp32 PHASE A — from-scratch replication of exp22-fovi-teacherinit at the CURRENT code base.
#
# Replicates jon_exp22_full_runs/exp22-fovi-teacherinit config-for-config. Every CFG_ value and
...
#
# PHASE A = warmup(100k) -> constant 4e-4, exactly as exp22. It stops at step 1130496;
# the x0.1 LR drop is a SEPARATE launcher (see exp32-fovi-teacherinit-lrdrop.sh) that seeds from this
# run's step-1130496.pt. There is no in-run LR-drop feature and the drop point is therefore
# a FILENAME, not a step comparison -- it cannot fire early if array tasks fail.
#
# ARRAY = 138 x 8192 = 1130496 steps.
...
# schedule: if tasks die, re-submit the remainder until step-1130496.pt exists, and only
