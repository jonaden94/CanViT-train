#!/bin/bash
# exp32 PHASE A — from-scratch replication of exp22-uniform16 at the CURRENT code base.
#
# Replicates jon_exp22_full_runs/exp22-uniform16 config-for-config. Every CFG_ value and
...
#
# PHASE A = warmup(100k) -> constant 4e-4, exactly as exp22. It stops at step 1441792;
# the x0.1 LR drop is a SEPARATE launcher (see exp32-uniform16-lrdrop.sh) that seeds from this
# run's step-1441792.pt. There is no in-run LR-drop feature and the drop point is therefore
# a FILENAME, not a step comparison -- it cannot fire early if array tasks fail.
#
# ARRAY = 176 x 8192 = 1441792 steps.
...
# schedule: if tasks die, re-submit the remainder until step-1441792.pt exists, and only
# then launch the lrdrop phase.
set -euo pipefail

# === ESSENTIALS ===
RUN_GROUP=jon_exp32_pretrain_lrdrop
RUN_NAME=exp32-uniform16
ARRAY=0-175%1
TIME=0-02:00:00
MEM=128G
NGPU=1
