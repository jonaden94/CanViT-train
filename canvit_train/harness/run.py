...
publishing stays the manual ``python -m canvit_train.checkpoint.to_hf`` step.
"""
...
# run.py IS the harness entry point (`python -m canvit_train.harness.run`), so guarding
...
def run(*, task: RunTask, spec: TrainSpec, settings: RunSettings) -> dict:
    ...
    from canvit_train.checkpoint import current_provenance
...
def main(argv: list[str] | None = None) -> None:
    """``python -m canvit_train.harness.run {distill,ade20k,in1k} [flags]``.

    The CLI itself lives in :mod:`canvit_train.harness.cli` — tyro over each task's
    ...
    from canvit_train.harness.cli import main as cli_main
