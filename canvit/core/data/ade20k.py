"""ADE20K-SceneParse150 dataset + validation transforms.

Shared home for the ADE20K primitives used by BOTH the training repo
(CanViT-pretrain's segmentation probe) and CanViT-eval. Living in core means a
single source of truth for the val protocol (squish/center-crop resize, 1-indexed
mask handling) that every consumer's numbers are anchored to. The training-only
augmentation/optimizer pipeline stays in CanViT-pretrain; only the dataset class,
the val transforms, and the label constants are shared here.

The ImageNet normalization constants are inlined (identical to
``timm.data.IMAGENET_DEFAULT_MEAN/STD``) so core carries no timm dependency.
"""
...
# Identical to timm.data.IMAGENET_DEFAULT_MEAN / IMAGENET_DEFAULT_STD.
